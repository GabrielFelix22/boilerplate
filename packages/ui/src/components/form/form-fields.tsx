...
type FormFieldProps<TFieldValues extends FieldValues = FieldValues> = {
  ...
  children: (args: { field: any; disabled: boolean; id: string }) => ReactNode;
};
      ...
      {({ field, disabled, id }) => (
        <Input
          ...
          onChange={(event) => {
            ...
            const nextValue = transformValue ? transformValue(raw) : raw;
...
type DateFieldProps = {
  ...
  fieldClassName?: string;
  disabled?: boolean;
} & Omit<React.ComponentProps<typeof DateInput>, 'value' | 'onChange' | 'id'>;
    ...
    >
      {({ field, disabled: formDisabled, id }) => (
        <RadioGroup
          ...
          disabled={disabled ?? formDisabled}
          className={
            layout === 'horizontal' ? 'flex flex-row gap-4' : undefined
          }
        >
          {options.map((option) => (
