...
export function DataTableFilterMenu<TData>({
    ...
    const debouncedSetFilters = useDebouncedCallback(
        (next: ExtendedColumnFilter<TData>[] | ((prev: ExtendedColumnFilter<TData>[]) => ExtendedColumnFilter<TData>[])) => {
            const prev = (table.getState().columnFilters ?? []) as ExtendedColumnFilter<TData>[]
            const value = typeof next === 'function' ? next(prev) : next
            table.setColumnFilters(value as any)
        },
        debounceMs,
    )
            ...
            <Popover open={open} onOpenChange={onOpenChange}>
                <PopoverTrigger asChild>
                    <Button
                        ...
                        className={cn(filters.length > 0 && "size-8", "h-8 font-normal gap-2")}
                        ref={triggerRef}
...
function DataTableFilterItem<TData>({
...
}: DataTableFilterItemProps<TData>) {
    {
        ...
        // Ensure value selector is closed for operators that don't take a value
        React.useEffect(() => {
            ...
            >
                ...
                <Select
                    ...
                    onValueChange={(value: FilterOperator) => {
                        ...
                        if (value === 'isBetween') setShowValueSelector(true)
                        if (value === 'isEmpty' || value === 'isNotEmpty') setShowValueSelector(false)
                    }}
                >
                    <SelectTrigger
                ...
                <CommandGroup>
                    ...
                    >
                        {isEmpty ? (
                            <>
                                <Text />
                                <span>Digite para adicionar filtro...</span>
                            </>
                        ) : (
                            <>
                                <BadgeCheck />
